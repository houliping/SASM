...
class ATSSCFAHead(nn.Module):
    def __init__(self,
        ...
        super(ATSSCFAHead, self).__init__()
    ...
    def cfa_reassign(self, pos_losses, label, label_weight, pts_pred_init, convex_weight, gt_rbbox,
        ...
        for gt_ind in range(num_gt):
            ...
            if len(pos_inds_cfa) < 2:
                ...
                re_assign_weights = reweights.reshape(-1) / reweights.sum() * torch.ones(len(reweights)).type_as(
                    gauss_prob_norm).sum()
    ...
    def point_target_refine_single(self,
        ...
        if sampling:
            ...
            if cfg.assigner.type != "ATSSAssigner" and cfg.assigner.type != "ATSSConvexAssigner" \
                    and cfg.assigner.type != "LAConvexAssigner":
                assign_result = bbox_assigner.assign(proposals, gt_rbboxes, overlaps,
                                                     gt_rbboxes_ignore, gt_labels)
            else:
