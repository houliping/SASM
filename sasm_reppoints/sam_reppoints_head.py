...
from mmdet.core.bbox.transforms_points import points_center_pts, rbboxes_center_pts
from mmdet.ops.dis_pointLine import disPL
...
class SAMRepPointsHead(nn.Module):
    def __init__(self,
        ...
        super(SAMRepPointsHead, self).__init__()
    ...
    def loss_single(self, cls_score, pts_pred_init, pts_pred_refine, labels,
                    ...
                    sam_weights_init,
                    rbbox_gt_refine, convex_weights_refine,
        ...
        # if use sam
        # loss_cls = loss_cls / sam_weights_refine.sum(-1)
    ...
    def loss(self,
        ...
        if cfg.init.assigner['type'] == 'ConvexAssigner':
            ...
            raise NotImplementedError
        cls_reg_targets_init = self.sam_point_target(
            candidate_list,
            valid_flag_list,
            gt_rbboxes,
            img_metas,
        ...
        (*_, rbbox_gt_list_init, candidate_list_init, convex_weights_list_init,
         num_total_pos_init, num_total_neg_init, gt_inds_init, sam_weights_list_init) = cls_reg_targets_init
        ...
        cls_reg_targets_refine = self.sam_point_target_refine(
            points_list,
            valid_flag_list,
            gt_rbboxes,
            img_metas,
        ...
        (labels_list, label_weights_list, rbbox_gt_list_refine,
         ...
         num_total_neg_refine, gt_inds_refine,
        ...
        #  if use sam
        # losses_cls = losses_cls / sam_weights_refine
        # losses_pts_refine = losses_pts_refine / sam_weights_refine
    ...
    def sam_point_target(self,
        ...
        (all_labels, all_label_weights, all_rbbox_gt, all_proposals,
         ...
         all_gt_inds_list, all_sam_init_weights) = multi_apply(
            self.sam_point_target_single,
            proposals_list,
            valid_flag_list,
            gt_rbboxes_list,
        ...
        sam_init_weights_list = images_to_levels(all_sam_init_weights, num_level_proposals)



        return (labels_list, label_weights_list, rbbox_gt_list, proposals_list,
                proposal_weights_list, num_total_pos, num_total_neg, gt_inds_list, sam_init_weights_list)

    def sam_point_target_single(self,
                            flat_proposals,
                            valid_flags,
                            gt_rbboxes,
                            gt_rbboxes_ignore,
        ...
        # use la
        rbboxes_center, width, height, angles = rbboxes_center_pts(rbbox_gt)
        ...
        distances[angles_index_wh] = torch.sqrt(
            (torch.pow(rbboxes_center[angles_index_wh, 0] - points_xy[angles_index_wh, 0], 2) / width[
                angles_index_wh])
            + (torch.pow(rbboxes_center[angles_index_wh, 1] - points_xy[angles_index_wh, 1], 2) / height[
                angles_index_wh]))

        # debug
        distances_wh = distances[angles_index_wh]

        distances[angles_index_hw] = torch.sqrt(
            (torch.pow(rbboxes_center[angles_index_hw, 0] - points_xy[angles_index_hw, 0], 2) / height[
                angles_index_hw])
            + (torch.pow(rbboxes_center[angles_index_hw, 1] - points_xy[angles_index_hw, 1], 2) / width[
                angles_index_hw]))
        distances[distances == float('nan')] = 0.

        sam_weights_init = label_weights * (torch.exp(1 / (distances + 1)))
        sam_weights_init[sam_weights_init == float('inf')] = 0.
        ...
        if unmap_outputs:
            ...
            sam_weights_init = unmap(sam_weights_init, num_total_proposals,
                                      inside_flags)

        return (labels, label_weights, rbbox_gt, pos_proposals, proposals_weights,
                pos_inds, neg_inds, gt_inds, sam_weights_init)
    ...
    def refine_point_target(self,
        ...
        (all_labels, all_label_weights, all_rbbox_gt, all_proposals,
         all_proposal_weights, pos_inds_list, neg_inds_list, all_gt_inds) = multi_apply(
            self.refine_point_target_single,
            proposals_list,
            valid_flag_list,
            gt_rbboxes_list,
            gt_rbboxes_ignore_list,
    ...
    def refine_point_target_single(self,
                                flat_proposals,
                                valid_flags,
                                gt_rbboxes,
                                gt_rbboxes_ignore,
    ...
    def sam_point_target_refine(self,
                            proposals_list,
                            valid_flag_list,
                            gt_rbboxes_list,
                            img_metas,
        ...
        (all_labels, all_label_weights, all_rbbox_gt, all_proposals,
         ...
         all_sam_weights) = multi_apply(
            self.sam_point_target_refine_single,
            proposals_list,
        ...
        proposal_weights_list = images_to_levels(all_proposal_weights,
                                                 num_level_proposals)
        gt_inds_list = images_to_levels(all_gt_inds_list, num_level_proposals)
        sam_weights_list = images_to_levels(all_sam_weights, num_level_proposals)
        return (labels_list, label_weights_list, rbbox_gt_list, proposals_list,
                proposal_weights_list, num_total_pos, num_total_neg, gt_inds_list, sam_weights_list)

    def sam_point_target_refine_single(self,
                                   flat_proposals,
                                   num_level_proposals,
                                   valid_flags,
                                   gt_rbboxes,
        ...
        if sampling:
            ...
            if cfg.assigner.type != "ATSSAssigner" and cfg.assigner.type != "ATSSConvexAssigner" \
                    and cfg.assigner.type != "SASAssigner":
                assign_result = bbox_assigner.assign(proposals, gt_rbboxes, overlaps,
                                                     gt_rbboxes_ignore, gt_labels)
            else:
        ...
        # if use la
        rbboxes_center, width, height, angles = rbboxes_center_pts(rbbox_gt)
        points_center = points_center_pts(pos_proposals, y_first=True)

        distances = torch.zeros_like(angles).reshape(-1)

        # angles_index = (width != 0)
        # angles = angles[angles_index]
        # # 02_la:compution of distance
        # distances_output_all_proposals_gt = torch.full([points_center.size(0), rbbox_gt.size(0), 4], 0.).cuda().float()
        # disPL(points_center[pos_inds], rbbox_gt[pos_inds], distances_output_all_proposals_gt[pos_inds])
        # distances_output_pos_proposals_gt = torch.diagonal(distances_output_all_proposals_gt, dim1=-3, dim2=-2)
        # distances_output = distances_output_pos_proposals_gt.transpose(0, 1)
        ...
        # 01_la:compution of distance
        distances[angles_index_wh] = torch.sqrt(
                (torch.pow(rbboxes_center[angles_index_wh, 0] - points_center[angles_index_wh, 0], 2) / width[angles_index_wh])
                + (torch.pow(rbboxes_center[angles_index_wh, 1] - points_center[angles_index_wh, 1], 2) / height[angles_index_wh]))

        # debug
        distances_wh = distances[angles_index_wh]

        distances[angles_index_hw] = torch.sqrt(
                (torch.pow(rbboxes_center[angles_index_hw, 0] - points_center[angles_index_hw, 0], 2) / height[angles_index_hw])
                + (torch.pow(rbboxes_center[angles_index_hw, 1] - points_center[angles_index_hw, 1], 2) / width[angles_index_hw]))
        # if debug
        distances_hw = distances[angles_index_hw]

        # # 02_la:compution of distance
        #
        # dis_p0p2_min = torch.min(distances_output[..., 0], distances_output[...,  2])
        # dis_p1p3_min = torch.min(distances_output[..., 1], distances_output[...,  3])
        #
        # distances[angles_index_wh] = torch.sqrt(
        #     (torch.pow(width[angles_index_wh]/2 - dis_p0p2_min[angles_index_wh], 2) / width[
        #         angles_index_wh])
        #    + (torch.pow(height[angles_index_wh] / 2 - dis_p1p3_min[angles_index_wh], 2) / height[
        #         angles_index_wh]))
        #
        # distances[angles_index_hw] = torch.sqrt(
        #     (torch.pow(height[angles_index_hw]/2 - dis_p0p2_min[angles_index_hw], 2) / height[
        #         angles_index_hw])
        #    + (torch.pow(width[angles_index_hw] / 2 - dis_p1p3_min[angles_index_hw], 2) / width[
        #         angles_index_hw]))
        ...
        if unmap_outputs:
            ...
            proposals_weights = unmap(proposals_weights, num_total_proposals,
                                      inside_flags)
            sam_weights = unmap(sam_weights, num_total_proposals,
                                      inside_flags)
            gt_inds = unmap(gt_inds, num_total_proposals, inside_flags)

        return (labels, label_weights, rbbox_gt, pos_proposals, proposals_weights,
                pos_inds, neg_inds, gt_inds, sam_weights)
